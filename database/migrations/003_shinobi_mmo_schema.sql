...
CREATE TABLE IF NOT EXISTS IF NOT EXISTS villages (
...
CREATE TABLE IF NOT EXISTS IF NOT EXISTS players (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- Player stats table
CREATE TABLE IF NOT EXISTS IF NOT EXISTS player_stats (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS IF NOT EXISTS player_progress (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    player_id UUID NOT NULL REFERENCES players(id) ON DELETE CASCADE,
...
-- Transactions: recent transactions by player
CREATE INDEX IF NOT EXISTS transactions_player_recent 
...
-- Note: Materialized views don't support RLS directly, but the underlying tables (players, player_progress) have RLS enabled
...
CREATE POLICY "Clan members can view clan data" ON clans
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE cm.clan_id = clans.id AND p.user_id = auth.uid()
        )
    );
...
CREATE POLICY "Clan members can view clan messages" ON clan_messages
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM clan_members cm 
            JOIN players p ON cm.player_id = p.id 
            WHERE cm.clan_id = clan_messages.clan_id AND p.user_id = auth.uid()
        )
    );
...
CREATE POLICY "Clan chat readable by clan members" ON chat_messages
    FOR SELECT USING (
        ...
        EXISTS (
            SELECT 1 FROM clan_members cm 
            JOIN players p ON cm.player_id = p.id 
            WHERE cm.clan_id = chat_messages.clan_id AND p.user_id = auth.uid()
        )
    );
...
INSERT INTO villages (name, description, coordinates) VALUES
('Konohagakure', 'The Village Hidden in the Leaves', POINT(0, 0)),
('Sunagakure', 'The Village Hidden in the Sand', POINT(100, 0)),
('Kirigakure', 'The Village Hidden in the Mist', POINT(0, 100)),
('Iwagakure', 'The Village Hidden in the Stone', POINT(-100, 0)),
('Kumogakure', 'The Village Hidden in the Clouds', POINT(0, -100));
